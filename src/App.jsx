...
function Dashboard({ setSelectedCustomer }) {
  const magazines = [...new Set(CUSTOMERS.map((c) => c.magazine))];
  ...
  const salesBreakdown = magazines.map((m) => ({
    name: m,
    value: CUSTOMERS.filter(
      (c) => c.magazine === m && c.status === "Buyer"
    ).reduce((s, c) => s + c.amount, 0),
  }));

  const customerBreakdown = magazines.map((m) => ({
    name: m,
    value: CUSTOMERS.filter((c) => c.magazine === m).length,
  }));

  const viewerBreakdown = magazines.map((m) => ({
    name: m,
    value: CUSTOMERS.filter((c) => c.magazine === m && c.status === "Viewer")
      .length,
  }));

  const buyerBreakdown = magazines.map((m) => ({
    name: m,
    value: CUSTOMERS.filter((c) => c.magazine === m && c.status === "Buyer")
      .length,
  }));
  ...
  const topCustomers = useMemo(() => {
    ...
    CUSTOMERS.filter((c) => c.status === "Buyer").forEach((c) => {
      totals[c.email] = (totals[c.email] || 0) + c.amount;
    });
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {filteredCustomers.filter((c) => c.status === "Buyer").length}
              </p>
            </div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {filteredCustomers.filter((c) => c.status === "Viewer").length}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-600">{customer.location}</span>
              </div>
              <div className="flex items-center gap-3">
                <BookOpen className="text-slate-400" size={16} />
                <span className="text-slate-600">{customer.magazine}</span>
              </div>
...
function Overview() {
  const magazineStats = [...new Set(CUSTOMERS.map((c) => c.magazine))].map(
    (magazine) => {
      const customers = CUSTOMERS.filter((c) => c.magazine === magazine);
      const buyers = customers.filter((c) => c.status === "Buyer");
      const viewers = customers.filter((c) => c.status === "Viewer");
      const revenue = buyers.reduce((sum, c) => sum + c.amount, 0);
  ...
  const totalRevenue = magazineStats.reduce((sum, mag) => sum + mag.revenue, 0);
  const avgConversion =
